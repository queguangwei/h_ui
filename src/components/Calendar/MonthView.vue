<template>
  <div :class="[`${prefixCls}-month-view ${prefixCls}-month-wrapper clearfix`]" ref="month">
    <div
      v-for="(monthInfo, index) in curMonthList"
      :key="index"
    ...
    >
      <div>
        <label
          ...
          :style="labelStyle"
        >{{t(`i.calendar.month.${'m' + (monthInfo.month + 1)}`)}}</label>
        <div :class="[`${prefixCls}-month-content ${prefixCls}-month-week`]">
          <div
            :class="[`${prefixCls}-month-content-item`]"
            :style="weekdayStyle"
            v-for="val in weekday"
            :key="val"
          >{{t(`i.calendar.week.${'w' + val}`)}}</div>
        </div>
        <div :class="[`${prefixCls}-month-content ${prefixCls}-month-day`]">
          <date-cell
            v-for="(item, index) in monthInfo.list"
            :key="index"
            :style="cellStyle"
            :prefixCls="prefixCls"
            :data="item"
...
export default {
  ...
  computed: {
    ...
    monthStyle() {
      ...
      return {
        width: (monthViewNum === 4 || monthViewNum === 2) ? '47%' : (monthViewNum === 1 ? '97%' : '22%')
      }
    },
    cellStyle() {
      const monthViewNum = this.monthViewNum;
      return {
        height: (monthViewNum === 4 || monthViewNum === 2) ? '50px' : (monthViewNum === 1 ? '100px' : '36px')
      }
    },
    weekdayStyle() {
      const monthViewNum = this.monthViewNum;
      return {
        height: (monthViewNum === 4 || monthViewNum === 2) ? '40px' : (monthViewNum === 1 ? '45px' : '36px')
      }
    },
    labelStyle() {
      const monthViewNum = this.monthViewNum;
      let height = (monthViewNum === 4 || monthViewNum === 2) ? '40px' : (monthViewNum === 1 ? '55px' : '36px');
      return {
        height: height,
        lineHeight: height
      }
