<template>
  <div>
    <h2>基础</h2>
    <h-table :columns="columns1" :data="data1" size="small" :disabled-hover="true" :highlight-row="true" @on-current-change="click1"></h-table>
    <h2>带边线</h2>
    <h-table border :columns="columns1" :data="data0" stripe no-data-text="数据为空" :show-header="false">
    ...
    <p>自定义行样式：</p>
    <h-table :row-class-name="rowClassName" :columns="columns1" :data="data1"></h-table>
    <p>自定义列样式：</p>
    <h-table :columns="columns9" :data="data1"></h-table>
    <p>自定义任意单元格样式：</p>
    <h-table :columns="columns1" :data="data8" @on-row-click="click1"></h-table>
    <h2>固定表头</h2>
    <p>通过设置属性 height 给表格指定高度后，会自动固定表头。当纵向内容过多时可以使用</p>
    <h-table height="200" :columns="columns1" :data="data2" border></h-table>
    <h2>固定列</h2>
    <p>通过给数据 columns 的项设置 fixed 为 left 或 right，可以左右固定需要的列。</p>
    <h-table width="1000" border :columns="columns2" :data="data3"></h-table>
    <h2>固定表头和列</h2>
    <h-table width="550" height="300" border :columns="columns2" :data="data4"></h-table>
    <h2>单选</h2>
    <p>通过设置属性 highlight-row，可以选中某一行。</p>
    <p>给 data 项设置特殊 key _highlight: true 可以默认选中当前项。</p>
    <h-table highlight-row :columns="columns3" :data="data1" @on-current-change='radioChange'></h-table>
    <h2>多选</h2>
    ...
    <h-table border :columns="columns4" :data="data1" @on-select-all="allSelect" @on-select="select" :rowSelect="true" @on-selection-change="selsetChange"></h-table>
    <h2>排序</h2>
    ...
    <h-table border :columns="columns5" :data="data5" @on-sort-change='sortChange'></h-table>
    <h2>筛选</h2>
    ...
    <h-table :columns="columns10" :data="data9" @on-expand="expand" @on-row-click="rowclick" @on-row-dblclick="rowdblclick"></h-table>
    <h2>设置大小</h2>
    ...
    <h-table height="300" border :columns="columns8" :data="data7"></h-table>
    <br>
    ...
    <h2>测试</h2>
    <h-table height="300" :stripe="true" :columns="columns18" :data="data17" border size="small" ref="table" :rowSelect="true">
         <span slot="header">证券日活数据表</span>
         <span slot="footer">恒生电子有限公司提供</span>
    </h-table>
    <br>
    <h-table border :columns="columns6" :data="data5" no-filtered-data-text="找不到数据"></h-table>
  </div>
</template>
<script>
